...
def render_ackermann(data, var_name="s", mat_name="A", col_name="B",
                     u_label=r"\mathcal{U}", gain_label="K"):
    """Renderiza TODOS os passos de Ackermann (K ou L dual)."""
    n = data["n"]
    ...
    st.markdown("---")
    st.markdown(f"**Passo 3 – Substituição de ${mat_name}$ no polinômio "
                r"(Teorema de Cayley-Hamilton)**")
    terms_str_parts = []
...
if page.startswith("1"):
    ...
    with tab_k:
        if st.button("▶  Calcular K", key="btn2k"):
            try:
                ...
                data = ackermann_gain_steps(A_np, B_np, A_sym, B_sym, poles)
    ...
    if st.button("▶  Calcular Seguidor", key="btn3"):
        try:
            ...
            st.markdown("Adicionamos um estado integrador $\\xi$ tal que "
                        "$\\dot{\\xi} = r - Cx$.")
    ...
    if st.button("▶  Converter", key="btn4"):
        try:
            ...
            st.markdown("$A_{ocf} = A_{ccf}^T$, $B_{ocf} = C_{ccf}^T$, "
                        "$C_{ocf} = B_{ccf}^T$.")
    ...
    st.info("Converte $(A, B)$ contínuo em $(G, H)$ discreto para um dado "
    ...
    if st.button("▶  Discretizar", key="btn5"):
        try:
            ...
            sys_c = ctrl.ss(A_np, B_np, np.eye(A_np.shape[0]),
                            np.zeros_like(B_np))
